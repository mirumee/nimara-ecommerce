...
/**
 * Renders the config UI outside the Dashboard iframe. Both reads sit behind
 * `import.meta.env.DEV`, so a production bundle carries no token.
 */
export const readDevSession = () => {
  ...
  return { accessToken, saleorApiUrl };
};
