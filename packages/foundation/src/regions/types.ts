...
> = Lowercase<keyof C["markets"] & string>;
export type LanguageIdOf<
  C extends { languages: Readonly<Record<string, unknown>> },
> = Lowercase<keyof C["languages"] & string>;
export type SupportedCurrencyOf<
  C extends { supportedCurrencies: readonly string[] },
> = C["supportedCurrencies"][number];
...
export type LanguageOf<
  C extends { languages: Readonly<Record<string, unknown>> },
> = C["languages"][keyof C["languages"]];
export type MarketOf<C extends { markets: Readonly<Record<string, unknown>> }> =
  C["markets"][keyof C["markets"]];
export type RegionOf<
  C extends {
