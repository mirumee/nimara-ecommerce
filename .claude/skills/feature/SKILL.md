...
1. **Locate** — search for similar code and patterns first. Reuse before adding. Read
   `.claude/skills/project-guidelines/SKILL.md` to decide the layer.
...
3. **Build** — use the relevant scoped `CLAUDE.md`. When delegating to a specialist agent,
   ...
   work without invoking `/pre-ship`. Wrap fallible operations in `Result`. Add `next-intl`
...
Never invoke `/pre-ship` from this workflow. Shipping is a separate developer-only action.
