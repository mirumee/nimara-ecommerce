...
- Run **local Lighthouse** (`npx lighthouse <url> --form-factor=mobile`) using the system Chrome (`CHROME_PATH`). Use the default throttled mobile profile unless the verification scenario defines another profile.
...
Playwright browser automation (navigation, snapshots, input, screenshots, CDP, and DOM evaluation), Bash and curl, local Lighthouse, and repository search/read tools.
