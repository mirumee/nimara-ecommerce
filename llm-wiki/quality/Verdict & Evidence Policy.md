...
1. **Evidence only.** Every verdict is backed by a verification artifact such as a screenshot, measurement, JSON payload, or response capture. State the environment, channel, and build.
2. **Never fabricate** repro steps, data, or credentials. Missing input is an ASK, not a guess.
3. **Don't force a verdict.** If verification is ambiguous or blocked, record it as inconclusive, request the missing input, and flag it for review.
...
- **Partial fixes**: if a defect report describes two failures and only one is resolved, record a partial result and list exactly what remains.
