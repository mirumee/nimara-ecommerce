...
updated: "2026-07-15T11:10:59+02:00"
...
- E-2: Nimara's [Table Stakes vs Differentiators](../market/Table%20Stakes%20vs%20Differentiators.md) analysis classifies AI-assisted shopper recommendations as a credibility-floor capability, while UCP/MCP remains the separate strategic differentiator.
...
The storefront has classic query, filter, collection, and product discovery plus separate UCP
catalog endpoints. No maintained AI provider adapter, guided-discovery UI, catalog-grounding
layer, AI fallback, or related telemetry described by this PRD was observed. See
[Catalog And Discovery](../../system/capabilities/Catalog%20And%20Discovery.md).
The `untracked` status means the idea has no established delivery direction; it does not mean the
idea was rejected.
...
The primary business outcome belongs to Nimara: qualified evaluators accept the capability and independent teams deploy it without maintaining a fork. [Shopper](../personas/Shopper.md) usefulness is the experience quality bar, but store-level conversion, revenue uplift, and catalog-specific relevance targets remain the responsibility of each deployment. No historical adoption baseline is used for this PRD.
...
- US-1 ([Storefront Developer](../personas/Storefront%20Developer.md)): As a developer or agency evaluating Nimara, I want to see working natural-language discovery in the public demo, so that I can judge whether Nimara meets this modern storefront expectation before selecting it.
- US-2 ([Storefront Developer](../personas/Storefront%20Developer.md)): As a storefront developer, I want to enable the reference capability with my own catalog and provider account within one business day, so that I can validate it without building the full experience or maintaining a fork.
- US-3 ([Storefront Developer](../personas/Storefront%20Developer.md)): As a storefront developer, I want to replace the provider and adapt placement, presentation, locale, eligibility, and ranking through supported extension points, so that the capability fits different storefronts while remaining upgradeable.
- US-4 ([Shopper](../personas/Shopper.md)): As a shopper, I want to describe my need in natural language and receive a concise list of current products with understandable reasons, so that I can reach relevant product pages without knowing the catalog's terminology.
- US-5 ([Shopper](../personas/Shopper.md)): As a shopper, I want an honest no-match response and a useful classic-search fallback when guided discovery cannot help, so that I never reach a dead end or see an invented product.
- US-6 ([Storefront Developer](../personas/Storefront%20Developer.md)): As a storefront developer, I want standard visibility into outcomes, failures, latency, provider usage, and cost, so that I can operate the capability within my deployment's budget and privacy policy.
...
[PRD-001 Natural-Language Product Discovery - Grilling Log](./grilling/PRD-001%20Natural-Language%20Product%20Discovery%20-%20Grilling%20Log.md)
