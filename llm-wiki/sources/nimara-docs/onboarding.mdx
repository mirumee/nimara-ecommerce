...
:::tip
Just want to run a store? Start with [Running Locally](./running-locally) and [Storefront](./storefront). Come back when you want to write code.
:::
...
Opens on [http://localhost:3000](http://localhost:3000). For webhooks and deployment, see the [Storefront guide](./storefront).
...
Run `pnpm preflight` to see what's currently on. The full list of variables is in [Environment Variables](./environment-variables).
...
- **[Running Locally](./running-locally)** — backend setup options.
- **[Storefront](./storefront)** — full setup, webhooks, deployment.
- **[Environment Variables](./environment-variables)** — every configurable variable.
- **[Saleor CMS](./saleor-cms)** & **[Add a new channel](./add-new-channel)** — content and markets.
- **[Stripe](./stripe-integration)**, **[UCP](./ucp-integration)**, **[Marketplace](./marketplace)** — integrations and advanced apps.
