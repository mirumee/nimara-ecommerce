...
import { type ResponseSchema, responseSchema } from "@/lib/api/schema";
...
/**
 * Base class for errors that map directly to an HTTP response. The error
 * handler serializes them with the same `responseSchema` shape the route
 * handlers use, so all error responses stay uniform.
 */
