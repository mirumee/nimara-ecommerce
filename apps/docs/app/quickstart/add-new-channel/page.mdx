...
```hcl filename="apps/storefront/src/regions/types.ts"
...
```hcl filename="apps/storefront/src/i18n/routing.ts"
...
```hcl filename="packages/domain/src/consts.ts":
// Add the new locale
export const ALL_LOCALES = ["en", "en-US", "en-GB", "ja-JP", "pl-PL", "es-ES"] as const;
```
...
```hcl filename="apps/storefront/src/regions/config.ts":
...
> = {
  "en-GB": "gb",
  "en-US": "us",
  "es-ES": "es", // Map the locale to the channel
};
