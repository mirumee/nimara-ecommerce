...
The marketplace app also uses the shared i18n package described in the [i18n architecture docs](/docs/i18n). It passes `app: "marketplace"` to `@nimara/i18n`’s `createRequestConfig`, so it receives the `common` + `marketplace` message bundles (with locale-specific overrides applied where defined).
