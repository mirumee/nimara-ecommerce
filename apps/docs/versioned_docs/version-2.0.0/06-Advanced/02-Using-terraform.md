...
- Access to the [Vercel account](https://vercel.com/account/settings)
...
These variables are essential for the storefront to function correctly, including Saleor app token, authentication secret, and Stripe secret key and must be kept private.
What they are and how to get them is described in the [Environment Variables](../Quickstart/environment-variables) section.
Vercel variables can be obtained from your Vercel account settings.

```hcl title="terraform/storefront/private.auto.tfvars"
...
Next, configure **public** environment variables in the same way.
...
```hcl title="terraform/storefront/public.auto.tfvars"
...
If everything looks good, apply the changes to deploy the storefront.
