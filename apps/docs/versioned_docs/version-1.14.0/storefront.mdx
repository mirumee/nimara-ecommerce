...
The storefront uses the shared i18n package and message composition system described in the [i18n architecture docs](/1.14.0/i18n). It passes `app: "storefront"` to `@nimara/i18n`'s `createRequestConfig`, so it receives the `common` + `storefront` message bundles (with locale-specific overrides applied where defined).
